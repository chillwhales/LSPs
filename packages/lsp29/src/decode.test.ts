...
describe('decodeLsp29Metadata', () => {
  ...
  it('should return correctly typed LSP29EncryptedAsset', () => {
    ...
    expect(result.LSP29EncryptedAsset.encryption.method).toBe('digital-asset-balance');
    expect(result.LSP29EncryptedAsset.chunks.ipfs?.cids).toEqual(['QmTest123', 'QmTest456']);
  });
  ...
  it('should throw on valid JSON but invalid schema (missing required fields)', () => {
    const invalidJson = JSON.stringify({ LSP29EncryptedAsset: { version: '2.0.0' } });
  ...
  it('should parse asset with lumera backend', () => {
    ...
    expect(result.LSP29EncryptedAsset.chunks.lumera?.actionIds).toEqual(['action-1', 'action-2']);
  });
  ...
  it('should parse asset with multiple backends', () => {
    ...
    expect(result.LSP29EncryptedAsset.chunks.lumera?.actionIds).toEqual(['action1']);
  });
});
