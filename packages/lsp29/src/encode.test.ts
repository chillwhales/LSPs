...
describe('computeLsp29ArrayIndexKey', () => {
  it('should compute key for index 0', () => {
    ...
    expect(key).toBe('0x1965f98377ddff08e78c93d820cc8de400000000000000000000000000000000');
  });

  it('should compute key for index 1', () => {
    const key = computeLsp29ArrayIndexKey(1);
    expect(key).toBe('0x1965f98377ddff08e78c93d820cc8de400000000000000000000000000000001');
  });

  it('should compute key for large index', () => {
    const key = computeLsp29ArrayIndexKey(1000);
    expect(key).toBe('0x1965f98377ddff08e78c93d820cc8de4000000000000000000000000000003e8');
  });
  ...
  it('should throw for negative index', () => {
    expect(() => computeLsp29ArrayIndexKey(-1)).toThrow('Index must be a non-negative integer');
  });
...
describe('Key Uniqueness', () => {
  it('should produce unique keys for different operations on same content', () => {
    ...
    const keys = new Set([latestMapKey, versionedMapKey, revisionCountKey, arrayIndexKey]);
