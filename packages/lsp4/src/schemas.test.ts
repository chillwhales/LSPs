...
describe("lsp4MetadataSchema", () => {
	...
	describe("array field validation", () => {
		...
		it("should validate array contents", () => {
			...
			expect(lsp4MetadataSchema.safeParse(invalidLinkContent).success).toBe(
				false,
			);
			expect(
				lsp4MetadataSchema.safeParse(invalidAttributeContent).success,
			).toBe(false);
		});
	});
	...
	describe("complex real-world scenarios", () => {
		it("should handle complex metadata with multiple attribute types", () => {
			const complexMetadata = {
				...
				icon: [{ ...validImage, width: 64, height: 64 }],
				images: [
	...
	describe("general validation", () => {
		...
		it("should provide meaningful error messages", () => {
			...
			if (!result.success) {
				const errors = result.error.issues.map((issue) => issue.message);
				expect(
					errors.some((error) => error.includes("Name must be a string")),
				).toBe(true);
			}
		});
	});
