...
const HEX_TO_DATA_KEY_NAME = new Map<string, string>();

/** Forward map: lowercase name → lowercase hex data key */
const DATA_KEY_NAME_TO_HEX = new Map<string, string>();
...
for (const [name, hex] of BUILT_IN_DATA_KEYS) {
	...
	HEX_TO_DATA_KEY_NAME.set(lowerHex, name);
	DATA_KEY_NAME_TO_HEX.set(name.toLowerCase(), lowerHex);
...
export function resolveDataKeyName(hex: string): string | null {
	...
	const exact = HEX_TO_DATA_KEY_NAME.get(lower);
...
export function resolveDataKeyHex(name: string): string | null {
	return DATA_KEY_NAME_TO_HEX.get(name.toLowerCase()) ?? null;
}
...
export function isKnownDataKeyName(name: string): boolean {
	return DATA_KEY_NAME_TO_HEX.has(name.toLowerCase());
}
...
export function isKnownDataKeyHex(hex: string): boolean {
	return HEX_TO_DATA_KEY_NAME.has(hex.toLowerCase());
}
...
export function getKnownDataKeyNames(): string[] {
	return [...HEX_TO_DATA_KEY_NAME.values()];
}
...
export function getKnownDataKeyHexes(): string[] {
	return [...HEX_TO_DATA_KEY_NAME.keys()];
}
...
export function getDataKeyCount(): number {
	return HEX_TO_DATA_KEY_NAME.size;
}
...
export function getDataKeyEntries(): [hex: string, name: string][] {
	return [...HEX_TO_DATA_KEY_NAME.entries()];
}
