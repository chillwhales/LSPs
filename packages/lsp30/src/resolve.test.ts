...
const allEntries: Lsp30Entry[] = [
  ...
  { backend: 's3', bucket: 'my-bucket', key: 'content/file.bin', region: 'us-east-1' },
...
describe('selectBackend', () => {
  ...
  it('should order by array preference: lumera first, ipfs second, then remaining', () => {
    ...
    expect(result[3]).toEqual({ backend: 'arweave', transactionId: 'tx-abc-def' });
    expect(result.length).toBe(allEntries.length);
  });
  ...
  it('should skip missing backends in preference and include present ones in order', () => {
    ...
    expect(result[1]).toEqual({ backend: 's3', bucket: 'b', key: 'k', region: 'r' });
    expect(result.length).toBe(entries.length);
  });
  ...
  it('should handle two entries of same backend type', () => {
    ...
    expect(result[2]).toEqual({ backend: 's3', bucket: 'b', key: 'k', region: 'r' });
...
describe('resolveUrl', () => {
  ...
  it('should return virtual-hosted S3 URL for S3 entry', () => {
    ...
    const url = resolveUrl(entry);
    expect(url).toBe('https://my-bucket.s3.us-east-1.amazonaws.com/content%2Ffile.bin');
  });
  ...
  it('should return arweave.net gateway URL for Arweave entry', () => {
    const entry: Lsp30Entry = { backend: 'arweave', transactionId: 'ABCDEF123456' };
    const url = resolveUrl(entry);
  ...
  it('should handle S3 keys with nested paths', () => {
    ...
    const url = resolveUrl(entry);
    expect(url).toBe('https://prod.s3.eu-west-1.amazonaws.com/encrypted%2F2024%2Fq1%2Fdata.bin');
  });
  ...
  it('should handle CIDv1 base32 IPFS CIDs', () => {
    ...
    const url = resolveUrl(entry);
    expect(url).toBe('ipfs://bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi');
  });
});
