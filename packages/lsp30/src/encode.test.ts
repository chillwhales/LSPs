import { Hex, hexToString, keccak256 } from 'viem';
...
// ============================================================================
// encodeLsp30Uri Tests
// ============================================================================

describe('encodeLsp30Uri', () => {
  it('should produce hex starting with 0x0030', () => {
    const encoded = encodeLsp30Uri(testEntries, testContentHash);
    expect(encoded.startsWith('0x0030')).toBe(true);
  });
  ...
  it('should reject entries with fewer than 2 items', () => {
    ...
    expect(() => encodeLsp30Uri(singleEntry, testContentHash)).toThrow();
  });

  it('should reject invalid verification hash format', () => {
    expect(() => encodeLsp30Uri(testEntries, '0xshort' as Hex)).toThrow();
  });
  ...
  it('should work with all 4 backend types', () => {
    ...
    const encoded = encodeLsp30Uri(allBackends, testContentHash);
    expect(encoded.startsWith('0x0030')).toBe(true);
  });
});
