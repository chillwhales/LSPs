...
Add `vitest` to root devDependencies and update the test script:
```json
{
  "devDependencies": {
    "vitest": "catalog:"
...
vitest must be in root devDependencies — pnpm strict isolation means child package deps are not resolvable from root. Without this, `vitest run` at root would fail with a missing binary.
...
5. `test ! -f vitest.workspace.ts` — deprecated vitest.workspace.ts file does not exist
6. `grep "vitest" package.json` — root package.json has vitest as devDependency
7. `pnpm test` from root — vitest discovers and runs all package tests
8. `cd packages/utils && pnpm test` — per-package test still works
  </verify>
  <done>
