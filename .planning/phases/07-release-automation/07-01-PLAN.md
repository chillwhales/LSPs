...
5. Check if knip reports @changesets/cli or @changesets/changelog-github as unused:
   ...
   pnpm knip
   ```
   If knip reports them as unused dependencies, add `ignoreDependencies` to the root workspace entry in `knip.json` (preserve existing structure under `workspaces`):
   ```json
   {
     "workspaces": {
       ".": {
   ...
   ```
   Only add the `ignoreDependencies` key — do NOT replace or remove existing fields (`entry`, `project`, `ignoreBinaries`, `packages/*`, `packages/config` entries).
