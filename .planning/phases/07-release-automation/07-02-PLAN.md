...
must_haves:
  truths:
    ...
    - "Preview workflow builds all packages and publishes snapshots via pkg-pr-new"
...
Purpose: Give PR reviewers installable preview URLs for changed packages — test before merge without waiting for npm publish.
...
<interfaces>
<!-- From Plan 01 — executor needs to know these exist -->
...
From .github/workflows/ci.yml (existing — preview.yml is a SEPARATE workflow):
- Trigger: push to main + pull_request to main
- 4-layer pipeline with build artifact passing

Packages to include in preview (all 8 public, exclude config which is private):
- packages/utils, packages/lsp2, packages/lsp3, packages/lsp4
- packages/lsp6, packages/lsp23, packages/lsp29, packages/lsp30
</interfaces>
</context>
...
<task type="auto">
  ...
  <action>
Create `.github/workflows/preview.yml` with the following exact structure. This workflow publishes snapshot releases via pkg-pr-new on every PR push, so reviewers can install and test changed packages before merge.
...
jobs:
  preview:
    ...
    # Forks could run malicious code — no npm tokens are used here but
...
- `on: pull_request` — triggers on every PR push (automatic, no opt-in needed per user decision)
- Fork guard: `if: github.event.pull_request.head.repo.full_name == github.repository` — blocks fork PRs from getting snapshots (security: per user decision, internal branches only)
- `'./packages/*'` glob — pkg-pr-new respects `private: true` in package.json, so @chillwhales/config is auto-excluded. Simpler than listing all 8 packages explicitly.
- `--compact` — generates short URLs like `https://pkg.pr.new/@chillwhales/utils@abc1234` (requires `repository` field in package.json — already present from Phase 6)
- `--comment=update` — posts one PR comment and updates it on subsequent pushes (not a new comment per push, per user decision)
- `--packageManager=pnpm` — shows `pnpm add` in install URLs instead of `npm i`
- No NPM_TOKEN needed — pkg-pr-new uses its own CDN, not npm registry
- No `permissions` block needed — inherits default read permissions (pkg-pr-new GitHub App handles comment posting)
- No concurrency group needed — stale previews are harmless and each run is independent
- Build step included — pkg-pr-new needs dist/ artifacts to publish snapshots

The resulting PR comment will look like:
```
...
1. **Changeset status check:**
   ...
   ```
   Expected: exits 0, reports "No changesets present" (correct — no changesets added yet).
...
5. **Existing tooling regression check:**
   ...
   pnpm check
   ```
   This runs biome check → sherif → knip → madge. All must exit 0.

6. **Build check:**
   ```
   pnpm build
   ```
   Must exit 0 — new configs should not affect build.

7. **Test check:**
   ```
   pnpm test
   ```
   Must exit 0 — new configs should not affect tests.
...
- knip reports changesets deps as unused → add to ignoreDependencies in knip.json
- sherif complains about new deps → check version consistency
  </action>
  <verify>
