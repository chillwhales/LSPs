...
must_haves:
  truths:
    - "Running pnpm test --coverage produces a merged coverage report in text, lcov, and HTML formats"
    ...
    - "Single-package coverage works via pnpm test:coverage -- --project=@chillwhales/<package>"
  artifacts:
...
Purpose: Enable coverage measurement with enforced 80% thresholds across all four metrics. This is the core deliverable of Phase 4 — coverage must work locally before CI can report it (Phase 5). Three report formats (text for terminal, lcov for CI/Codecov, HTML for local browsing) are generated simultaneously.
...
Per-package coverage should be run from the root using Vitest's project selection, for example: `pnpm test:coverage -- --project @chillwhales/lsp2`. Do not rely on `pnpm --filter` for `test:coverage` since per-package configs use `defineProject()` which does not support coverage. Do NOT add `test:coverage` to individual package.json files.
