...
**CRITICAL:** Use `test.projects` — NOT `vitest.workspace.ts`. The workspace config was deprecated in Vitest 3.2 and removed in 4.0. The `test.projects` API in `vitest.config.ts` is the Vitest 4.x way. The `!packages/config` exclusion prevents vitest from trying to discover tests in the config package (which has none).
